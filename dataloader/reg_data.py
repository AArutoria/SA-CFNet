...
class RegData(torch.utils.data.Dataset):
    ...
    def __init__(self, ir_folder: pathlib.Path, it_folder: pathlib.Path, ir_true_folder: pathlib.Path, crop=lambda x: x):
        ...
        self.ir_true_list = [x for x in sorted(it_folder.glob('*')) if x.suffix in ['.png', '.jpg', '.bmp']]
    ...
    def __getitem__(self, index):
        ...
        ir_true_path = self.ir_true_list[index]
        ...
        ir_true = self.imread(path=ir_true_path, flags=cv2.IMREAD_GRAYSCALE)
        ...
        return (ir, it,ir_true), (str(ir_path), str(it_path),str(ir_true_path))
    ...
    def imread(path: pathlib.Path, flags=cv2.IMREAD_GRAYSCALE):
        ...
        #     im_ts = im_ts.permute(1, 2, 0)  # (H, W, C) -> (C, H, W)
        return im_ts

...
def remap_using_flow_fields(image, disp_x, disp_y, interpolation=cv2.INTER_LINEAR, border_mode=cv2.BORDER_CONSTANT):
    ...
    """
    h_scale, w_scale=image.shape[:2] #[256]

    # estimate the grid
    X, Y = np.meshgrid(np.linspace(0, w_scale - 1, w_scale),
                       np.linspace(0, h_scale - 1, h_scale)) #(256,256)
    map_x = (X+disp_x).astype(np.float32)#[256,256]
    map_y = (Y+disp_y).astype(np.float32) #[256,256]

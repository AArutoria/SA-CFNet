...
from models.deformable_net import SSpatialTransformer
def hyper_args():
    """
    ...
    # dataset
    parser.add_argument('--ir', default=r'F:\five_layers\origin',
                        type=pathlib.Path)
    parser.add_argument('--it', default=r'F:\five_layers\reference_ir',
                        type=pathlib.Path)
    # parser.add_argument('--disp', default='../dataset/raw/ctest/Road/disp',    type=pathlib.Path)
    # checkpoint
    parser.add_argument('--ckpt', default=r'D:\UMF\result\RoadScene\UMF+transformer_mesloss+feature_inner\cp_0800.pth', help='weight checkpoint', type=pathlib.Path) # weight/default.pth
    parser.add_argument("--cuda", action="store_false", help="Use cuda?")
    parser.add_argument('--dst',  default=r'F:\five_layers\origin\origin_registration', help='fuse image save folder', type=pathlib.Path)
...
def main(args):
    ...
    spatial_transform_m.load_state_dict = (model_state_dict)

    print("===> Starting Testing")
    # test(net, test_data_loader, args.dst, device,affine,elastic,spatial_transform_m)
    test(net, test_data_loader, args.dst, device, spatial_transform_m)


# def test(net, test_data_loader, dst, device,affine,elastic,spatial_transform_m):
def test(net, test_data_loader, dst, device, spatial_transform_m):
    net.eval()
    ...
    for (ir, it), (ir_path, it_path) in tqdm_loader:
        ...
        start = time.time()
        # spatial_transform_m = spatial_transform_m((256, 256))
        with torch.no_grad():
            ir_pred, f_warp, flow, int_flow2, disp_pred = net(it, ir_warp)

        # ir_pred = spatial_transform_m(ir, flow)  # torch.Size([16, 16, 128, 128])
        # ir_warp = torch.concat((ir_warp,ir_warp,ir_warp),dim=1)
        # ir_warp = ir_warp.mean(0).permute(1, 2, 0).cpu().numpy()
        # warped_source_image = remap_using_flow_fields(ir_warp, flow.squeeze()[0].cpu().numpy(),
        #                                               flow.squeeze()[1].cpu().numpy())
        # warped_source_image = warped_source_image.transpose(2, 0, 1)
        # ir_pred = torch.Tensor(warped_source_image)
        # torch.cuda.synchronize() if str(device) == 'cuda' else None
        end = time.time()
        ...
        # imsave(ir, dst / 'IR_true', file_name)
        # imsave(it, dst / 'VIS' / file_name)
        imsave(ir_pred, dst / 'IR', file_name)

...
def STN(img, pre_tps): #IMG[8,1,256,256] pre_tps[8,6]
    ...
    img_reg = F.grid_sample(img, img_grid) #[8,1,256,256]
    return img_reg
...
class ADRNet(nn.Module):
    def __init__(self, config=None):
        ...
        self.RES = resnet()
        # self.RES = DCP()
        self.ST = SpatialTransformer(356,356,True)
        # self.UN = unet()
        ...
        # self.spatial_transform = SpatialTransformer(volsize=[256,256])
    ...
    # def test_forward(self, rgb, sar, sar_warp, gt_tp, gt_disp):
    def test_forward(self, rgb, sar, sar_warp):
        ...
        sw2r = self.RES(sar_stack, opt_stack)
        ...
        # sw2r_disp = sw2r_disp.permute(0,3,1,2)
        # rw2s_disp = rw2s_disp.permute(0,3,1,2)
        # rgb_reg_aff, sw2r_disp = self.spatial_transform(rgb_warp, rw2s_disp)  # torch.Size([16, 1, 256, 256]) torch.Size([16, 256, 256, 2])
        # sar_reg_aff, rw2s_disp = self.spatial_transform(sar_warp, sw2r_disp)
        # sw2r_disp = sw2r_disp.permute(0,2,3,1)
        # rw2s_disp = rw2s_disp.permute(0, 2, 3, 1)
        ...
        img_stack = sar_warp
        disp_stack = pre_disp1
        img_reg, flow = self.ST(img_stack, disp_stack)

        # image_sar_reg, image_rgb_reg = torch.split(img_reg, b, dim=0)
        image_sar_reg = img_reg

        image_sar_reg  = add_padding(image_sar_reg, padding_size=50)
        ...
        # return sar_reg_aff, loss_pts
        return sar_reg_aff

        # return rgb_reg_aff, sar_reg_aff,loss_pts,loss_disp
    ...
    def train_forward(self):
        ...
        sar_stack = self.image_sar_warp
        rgb_stack = self.image_rgb

        # self.sw2r, self.s2rw, self.r2sw, self.rw2s = self.RES(sar_stack, rgb_stack) #[8,6]

        self.sw2r = self.RES(sar_stack, rgb_stack)  # [8,6]


        _, self.sw2r_disp = affine_to_flow(self.sw2r, b) #[8,256,256,2]
        # _, self.rw2s_disp = affine_to_flow(self.rw2s, b)    # [8,256,256,2]

        # self.sw2r_disp = self.sw2r_disp.permute(0,3,1,2)
        # self.rw2s_disp = self.rw2s_disp.permute(0,3,1,2)
        # self.rgb_reg_aff, sw2r_disp = self.spatial_transform(self.image_rgb_warp, self.rw2s_disp)  # torch.Size([16, 1, 256, 256]) torch.Size([16, 256, 256, 2])
        # self.sar_reg_aff, rw2s_disp = self.spatial_transform(self.image_sar_warp, self.sw2r_disp)
        # self.sw2r_disp = sw2r_disp.permute(0,2,3,1)
        # self.rw2s_disp = rw2s_disp.permute(0, 2, 3, 1)

        self.mask = generate_mask(self.image_sar_warp*255) #[8,1,256,256]
        # _, self.gt_tp_disp = affine_to_flow(self.gt_tp, b)  # [8,256,256,2]
        self.mask_true = STN(self.mask, self.gt_tp)
        # self.gt_tp_disp = self.gt_tp_disp.permute(0,3,1,2)
        # self.mask_true,_ = self.spatial_transform(self.mask, self.gt_tp_disp)

        ########## self.image_rgb_warp [8,1,256,256] Self.rw2s[8,6]
        # self.rgb_reg_aff = STN(self.image_rgb_warp, self.rw2s)
        self.sar_reg_aff = STN(self.image_sar_warp, self.sw2r)
# ##############第二次迭代
#         self.sw2r = self.RES(self.sar_reg_aff, rgb_stack)  # [8,6]
#         _, self.sw2r_disp = affine_to_flow(self.sw2r, b)  # [8,256,256,2]
#         self.mask = generate_mask(self.sar_reg_aff * 255)
#         self.mask_true = STN(self.mask, self.gt_tp)
#         self.sar_reg_aff = STN(self.sar_reg_aff, self.sw2r)
#
# # ###########第三次迭代
#         self.sw2r = self.RES(self.sar_reg_aff, rgb_stack)  # [8,6]
#         _, self.sw2r_disp = affine_to_flow(self.sw2r, b)  # [8,256,256,2]
...
#         self.sar_reg_aff = STN(self.sar_reg_aff, self.sw2r)
        ...
        # # self.disp= self.UN(sar_stack_, rgb_stack_)  # [8,2,256,256]
        #
        # self.pre_disp_sw2r = self.sw2r_disp + self.disp['sar2rgb'].permute(0,2,3,1) #[B,256,256,2]
    ...
    def update_RF(self, image_rgb, image_sar, image_sar_warp, gt_tp, gt_disp):
    ...
    def backward_RF(self):
        ...
        grid = KU.create_meshgrid(h,w).cuda() #[B,256,256,2]
        idx3 = torch.Tensor([[[0,0,1]]]).cuda().repeat(b, 1, 1) #[B,1,3]
        unit = torch.Tensor([[[1,0,0],[0,1,0],[0,0,1]]]).cuda().repeat(b, 1, 1) #[B,3,3]

        sw2r = self.sw2r.reshape(-1, 2, 3) #[B,2,3]
        sw2r_mat = torch.cat((sw2r, idx3), dim=1) #[B,3,3]
        # r2sw = self.r2sw.reshape(-1, 2, 3) #[B,2,3]
        # r2sw_mat = torch.cat((r2sw, idx3), dim=1)  #[B,3,3]
        # e1 = torch.matmul(sw2r_mat, r2sw_mat)#[B,3,3]
        e1 = sw2r_mat#[B,3,3]

        # rw2s = self.rw2s.reshape(-1, 2, 3) #[B,2,3]
        # rw2s_mat = torch.cat((rw2s, idx3), dim=1)   #[B,3,3]
        # s2rw = self.s2rw.reshape(-1, 2, 3) #[B,2,3]
        # s2rw_mat = torch.cat((s2rw, idx3), dim=1) #[B,3,3]
        # e2 = torch.matmul(rw2s_mat, s2rw_mat) #[B,3,3]
                
        # dc1 = torch.sum(abs(e1-unit), dim=[-2,-1]).mean() + torch.sum(abs(e2-unit),dim=[-2,-1]).mean()
        dc1 = torch.sum(abs(e1-unit), dim=[-2,-1]).mean()
        # self.image_rgb[B,1256,256] self.image_rgb_reg[B,1,256,256] self.mask_true[B,1,256,256]
        # ld_loss = self.img_loss(self.image_rgb, self.rgb_reg_aff, self.mask_true) + \
        #           self.img_loss(self.image_sar, self.sar_reg_aff, self.mask_true)

        # self.sw2r[B,6] self.gt_tp[B,2,3]
        # loss_tp = torch.sum(abs(self.sw2r-self.gt_tp.reshape(b, -1))).mean() + torch.sum(abs(self.rw2s-self.gt_tp.reshape(b, -1))).mean()
        loss_tp = torch.sum(abs(self.sw2r-self.gt_tp.reshape(b, -1))).mean()
        ...
        loss_mi1 =  self.mi_loss(self.image_sar * self.mask_true, self.sar_reg_aff)
    ...
    def assemble_outputs(self):
        ...
        # images_ir = self.image_sar.detach()
        images_vi = self.image_rgb.detach()
        images_ir_warp = self.image_sar_warp.detach()
        # images_vi_warp = self.image_rgb_warp.detach()
        images_ir_Reg = self.sar_reg_aff.detach()
        # images_vi_Reg = self.rgb_reg_aff.detach()
        ...
        row1 = torch.cat(
            (images_vi[0:1, ::], images_ir_warp[0:1, ::], images_ir_Reg[0:1, ::]),3)
        # return row1

        return images_vi, images_ir_warp, images_ir_Reg
    # def assemble_outputs(self):
    #     images_ir = normalize_image(self.image_sar).detach()
    #     images_vi = normalize_image(self.image_rgb).detach()
    #     images_ir_warp = normalize_image(self.image_sar_warp).detach()

...
class ElasticTransform(nn.Module):
    ...
    def forward(self, input1):
        # generate noise
        batch_size, _, height, weight = input1.shape
        noise = torch.rand(batch_size, 2, height, weight) * 2 - 1
        # elastic transform
        input1_warped, disp = self.elastic_transform2d(input1, noise)



        # batch_size, _, height, weight = input2.shape
        # noise = torch.rand(batch_size, 2, height, weight) * 2 - 1
        # # elastic transform
        # warped2, disp2 = self.elastic_transform2d(input2, noise)
        return input1_warped,disp

...
class Saver():
    ...
    def write_img(self, ep, model):
        ...
        images_vi, images_ir_warp, images_ir_Reg = model.assemble_outputs()
        img_filename = '%s/train_ir_%05d.jpg' % (self.image_dir, ep)
        # torchvision.utils.save_image(assembled_images, img_filename, nrow=1)
        torchvision.utils.save_image(images_ir_Reg, img_filename, nrow=1)
        img_filename = '%s/train_ir_warped_%05d.jpg' % (self.image_dir, ep)
        # torchvision.utils.save_image(assembled_images, img_filename, nrow=1)
        torchvision.utils.save_image(images_ir_warp, img_filename, nrow=1)
        img_filename = '%s/train_vi_%05d.jpg' % (self.image_dir, ep)
        # torchvision.utils.save_image(assembled_images, img_filename, nrow=1)
        torchvision.utils.save_image(images_vi, img_filename, nrow=1)
